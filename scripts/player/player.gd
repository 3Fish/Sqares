...
func _apply_horizontal(delta: float) -> void:
	var dir := Input.get_axis("p%d_move_left" % player_id, "p%d_move_right" % player_id)
	if dir != 0.0:
...
func _handle_shoot() -> void:
	if not Input.is_action_pressed("p%d_shoot" % player_id):
		return
...
func _get_aim_direction() -> Vector2:
	...
	var stick := Vector2(
		Input.get_axis("p%d_aim_left" % player_id, "p%d_aim_right" % player_id),
		Input.get_axis("p%d_aim_up" % player_id,   "p%d_aim_down" % player_id),
	)
